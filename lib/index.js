...
internals.Connection.prototype.dropSegment = function (segment, callback) {

    if (!this.client) {
        return callback(new Error('Connection not started'));
    }

    const prefix = this.generateKey({ segment, id: '*' });

    this.client.keys(prefix, (keyErr, keys) => {

        if (keyErr) {
            return callback(keyErr);
        }

        this.client.del(keys.join(' '), (err) => {
